#!/usr/bin/env python3
"""Short real-LM end-to-end smoke test for normalized Huffman.

This script is intentionally not a benchmark experiment runner yet. It checks
that the second adapted stegomethod traverses the already shared infrastructure
without method-specific LM/transport code:
LM -> P_reference -> Huffman -> ordinary text -> retokenization -> decoder.
"""
...
def main() -> None:
    ...
    print("Huffman end-to-end smoke test")
    print("model:", config.model_id)
    print("revision:", config.revision)
    print("bits_per_word:", args.bits_per_word)
    print("candidate_count:", 1 << args.bits_per_word)
    ...
    # Normalized Huffman is deterministic once P_reference and secret bits are
    # fixed, so no method RNG/key is required for this smoke test.
    result = run_streaming_text_roundtrip(
        lm_adapter=lm,
        reference_builder=builder,
        method=HuffmanMethod(),
        method_config={"bits_per_word": args.bits_per_word},
    ...
    print()
    print("Huffman end-to-end pipeline: COMPLETED")
