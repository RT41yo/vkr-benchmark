...
Цель этапа 3 - проверить воспроизводимость базовых методов Bins, Huffman и Arithmetic Coding и убедиться, что их адаптация к архитектуре бенчмарка не изменила основную логику работы.
...
Итог этапа - проверка корректности воспроизведения базовых стегометодов и адаптации их к архитектуре бенчмарка.

---

## 3. Авторский эталон

Для всех трех методов используется зафиксированный репозиторий Harvard NLP:
...
Для воспроизведения Figure 3 используется **GPT-2 Medium (345M)**, поскольку именно эта модель указана в статье Ziegler et al., в которой описана авторская реализация.
...
- исходный внешний репозиторий остается неизменным;
...
Основная задача - воспроизвести форму кривых Figure 3 (Ziegler et al.) и сравнительный порядок методов.

### Шаг 4. Анализ расхождений

Отдельно проверяются причины расхождений с публикацией  Ziegler et al.

Особое внимание уделяется специальной точке Arithmetic Coding, для которой значение KL сильно зависит от `precision`.

Такие расхождения фиксируются как отдельный результат анализа.
...
Это направление используется для сопоставления с авторской реализацией и Figure 3 (Ziegler et al.).
...
- итоговый отчет и машиночитаемые результаты сформированы;
